#!/usr/bin/env python3
"""
circuitbreaker.py — Production-grade circuit breaker pattern implementation.

A self-contained, zero-dependency circuit breaker library with:
  • Three-state FSM: CLOSED → OPEN → HALF_OPEN → CLOSED
  • Configurable failure thresholds (count or rate)
  • Per-exception-type filtering (ignore certain errors)
  • Thread-safe state transitions with audit log
  • Bulkhead: concurrent call limiting per circuit
  • Metrics: rolling window success/failure/latency stats
  • Event hooks: on_open, on_close, on_half_open, on_success, on_failure
  • Decorator + context-manager + direct-call interfaces
...
------
  ...
  OPEN       — all calls fail fast (no upstream calls made)
  HALF_OPEN  — probe state; limited calls allowed through to test recovery

Usage
-----
  from circuitbreaker import CircuitBreaker, CircuitBreakerConfig, OpenCircuitError

  cb = CircuitBreaker("payments-api", config=CircuitBreakerConfig(
      failure_threshold=5,
  ...
  # Context manager
  with cb:
      response = requests.post("/charge", json={"amount": 42.0})

  # Direct call
  result = cb.call(requests.get, "https://api.example.com/health")
